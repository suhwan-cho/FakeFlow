...
def test_ytobj(model):
    ...
    for vos_data in test_loader:
        ...
        count = 0
        for i in range(masks.size(1)):
            tv.utils.save_image(vos_out['masks'][0, i].float().cpu(), 'outputs/YTOBJ/{}/{}/{}'.format(class_name, video_name, files[i][0].split('/')[-1]))
            if torch.sum(masks[0, i]) == 0:
                continue
            iou = iou + torch.sum(masks[0, i] * vos_out['masks'][0, i].cpu()) / torch.sum((masks[0, i] + vos_out['masks'][0, i].cpu()).clamp(0, 1))
            count = count + 1
...
def test_lvid(model):
    ...
    for vos_data in test_loader:
        ...
        count = 0
        for i in range(masks.size(1)):
            tv.utils.save_image(vos_out['masks'][0, i].float().cpu(), 'outputs/LVID/{}/{}'.format(video_name, files[i][0].split('/')[-1]))
            if torch.sum(masks[0, i]) == 0:
                continue
            iou = iou + torch.sum(masks[0, i] * vos_out['masks'][0, i].cpu()) / torch.sum((masks[0, i] + vos_out['masks'][0, i].cpu()).clamp(0, 1))
            count = count + 1
